...
//   if (arcs[n][i] || arcs[i][n]) throw new RuntimeException("Corrupted AdjacencyMatrix");
   ...
   **/ 
  public int[] getDirections(Location orig, Location dest, boolean avoidStairs, boolean avoidHills){
    //if avoidStairs is true, don't go down paths with stairs. same with avoid hills
    //need case for no path available
  ...
  public String directionsString(Location orig, Location dest, boolean avoidStairs, boolean avoidHills){
    int[] directions = getDirections(orig, dest, avoidStairs, avoidHills);
  ...
  public static void main(String[] args){
    ...
    int[] test = m.getDirections(a, e, false, false);
    for(int i = 0; i < test.length; i++){
      System.out.print(test[i] + "|");
    }
    
    System.out.println("\n" + m.directionsString(a, e, false, false));
    System.out.println("\n" + m.directionsString(a, b, false, false));
  }
